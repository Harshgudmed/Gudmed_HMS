...
// Exactly 552 items extracted from your PDF data
const catalogData = {
  // 254 Radiology items
  radiology: [
    'X RAY CHEST (Male)',
...
async function seed552CatalogData() {
  try {
    console.log(`\n${'='.repeat(70)}`)
    ...
    for (let i = 0; i < catalogData.radiology.length; i++) {
      ...
      try {
        ...
        errorCount++
        console.log(`⚠️  Radiology Error: ${examName} - ${err.message.substring(0, 60)}`)
      }
    }
    ...
    for (let i = 0; i < catalogData.pathology.length; i++) {
      ...
      try {
        ...
        errorCount++
        console.log(`⚠️  Pathology Error: ${testName} - ${err.message.substring(0, 60)}`)
      }
    }

    // Seed Surgery/Procedure items (stored in radiologyExam as procedures)
    for (let i = 0; i < catalogData.surgeryProcedure.length; i++) {
      ...
      try {
        ...
        errorCount++
        console.log(`⚠️  Surgery Error: ${procedureName} - ${err.message.substring(0, 60)}`)
      }
    }

...
const TYPE_RAIL = [
  ...
  { key: 'LAB', label: 'Lab', Icon: FlaskConical, accent: 'text-sky-600', ring: 'data-[on=true]:border-sky-500 data-[on=true]:bg-sky-50 data-[on=true]:text-sky-700' },
  { key: 'RADIOLOGY', label: 'Imaging', Icon: Scan, accent: 'text-indigo-600', ring: 'data-[on=true]:border-indigo-500 data-[on=true]:bg-indigo-50 data-[on=true]:text-indigo-700' },
                ...
                <CardContent className="py-3 flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className={TYPE_STYLE[o.orderType]} variant="secondary">{o.orderType}</Badge>
...
function NewOrderView({ admissionId, patientLabel, onClose, onPlaced }) {
  ...
  const [form, setForm] = useState({ priority: 'ROUTINE', quantity: 1, frequency: '', duration: '', route: '', notes: '' })
  ...
  const place = async () => {
    ...
    try {
      const res = await client.post('/inpatient', {
        ...
        frequency: form.frequency || null, duration: form.duration || null, route: form.route || null, notes: form.notes || null,
      })
                    ...
                    <button key={`${r.catalogModel}:${r.catalogItemId}`} onClick={() => setPicked(r)}
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block text-xs text-gray-400 truncate">
                          <Badge className={TYPE_STYLE[r.orderType]} variant="secondary">{r.orderType}</Badge>
                ...
                {!picked ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3 rounded-xl border-2 border-blue-200 bg-blue-50/40 p-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-gray-400">
                          <Badge className={TYPE_STYLE[picked.orderType]} variant="secondary">{picked.orderType}</Badge>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div><Label className="text-xs text-gray-500">Duration</Label><Input className="bg-white" placeholder="e.g. 5 days" value={form.duration} onChange={(e) => setForm((f) => ({ ...f, duration: e.target.value }))} /></div>
                    </div>
                    <div>
                    ...
                    <div><Label className="text-xs text-gray-500">Route</Label><Input className="bg-white" placeholder="PO / IV / IM / Topical" value={form.route} onChange={(e) => setForm((f) => ({ ...f, route: e.target.value }))} /></div>
                    <div><Label className="text-xs text-gray-500">Clinical notes</Label><Textarea className="bg-white" rows={2} placeholder="Indication / instructions…" value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} /></div>

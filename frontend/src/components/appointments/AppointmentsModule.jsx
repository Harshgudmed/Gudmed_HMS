            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <Form {...form}>
                ...
                >
                  ...
                  {/* OPD Service Selection */}
                  <FormField
                    control={form.control}
                    name="opdServiceId"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>OPD Service (Charge) *</FormLabel>
                        <Select
                          onValueChange={(value) => {
                            field.onChange(value);
                            const selected = opdServices.find((s) => s.id === value);
                            if (selected) {
                              form.setValue("consultationFee", String(selected.unitPrice));
                            }
                          }}
                        ...
                        >
                          <FormControl>
                            <SelectTrigger>
                              <SelectValue placeholder="Select OPD service & charge" />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                            {opdServices.map((service) => (
                              <SelectItem key={service.id} value={service.id}>
                                {service.serviceName} - ₹{service.unitPrice}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                        <p className="text-xs text-gray-400 mt-1">
                          Select the service type and associated charge
                        </p>
                        <FormMessage />
                      </FormItem>
                    )}
